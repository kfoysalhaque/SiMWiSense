...
monitor ="m2";  %%% Select different monitors to generate batches. Options are "m1" / "m2" / "m3"
Test = 'fine_grained';
...
for e = 1:length(env)
    for m = 1:length(activity)
         folder_name = sprintf('../Data/%s/%s/%s/%smo/%s/%s/', Test, env(e,:), BW, tot_mon, monitor, activity(m));
    ...
    files = dir(fullfile(folder_name, '*.mat'));
        ...
        for file_idx = 1:numel(files)
            ...
            for v = 1:length(packet_stop)
                ...
                folder_save = sprintf('../Data/%s/%s/%s/%smo/%s/Slots/%s/%s_batch/',Test, env(e,:), BW, tot_mon, monitor, slots(v), activity(m));
